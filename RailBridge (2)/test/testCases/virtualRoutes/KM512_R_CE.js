...
const onDataReceived = (info) => {
  ...
  if(info.connected) {
    if(currentState === STATE['WAITING_CONNECTION']) {
    ...
    else if(info.section === 3 && info.remote === 15 && info.port === 2 && info.value === 2) {
      // Cancel Route KM512.R_CE

      console.debug('Cancel route KM512.R_CE received !!!');
    ...
    else if(info.section === 3 && info.remote === 15 && info.port === 1 && info.value === 16) {
      // Requested Route KM512.R_CE
      console.debug('Requested Route KM512.R_CE !!!');

      if(currentState === STATE['LOCKED_ROUTE']) {
        console.debug(' -------- test case: LOCKED_ROUTE started !!!');

        //Send Route KM512.R_CD GRANTED - Real Route
        // console.debug('Send Route KM512.R_CD GRANTED - Real Route');
        // sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 2, value: 1}]});
  
        seq_indications = 1;
        setTimeout(() => {
          console.debug('Send Route KM512.R_CE GRANTED - Virtual Route');
          //Send Route KM512.R_CE GRANTED - Virtual Route
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 2, value: 1}, {index: 52, value: 128}]});
        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => { 
          console.debug('Send Indication KM512.WT Occup2 = true');
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 2, value: 0}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          //Send Indication KM512.WT Occup2 = false
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 13, value: 0}]});
      ...
      if(currentState === STATE['REAL_ROUTE_GRANTED']) {
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send Route KM512.R_CE REGISTRED - Virtual Route');
          //Send Route KM512.R_CE REGISTRED - Virtual Route
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 52, value: 0}, {index: 53, value: 1}]});
        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send Route KM512.R_CE AVAILABLE - Virtual Route');
          //Send Route KM512.R_CE AVAILABLE - Virtual Route
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 52, value: 0}, {index: 53, value: 0}]});
        }, INDICATION_INTERVAL * seq_indications);
      ...
      if(currentState === STATE['REAL_ROUTE_REGISTERED']) {
        ...
        console.debug('Send Route KM512.R_CD REGISTERED - Real Route');
        //Send Route KM512.R_CD REGISTERED - Real Route
        sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 2, value: 2}]});
      ...
      if(currentState === STATE['REAL_ROUTE_TIMELOCKED']) {
        ...
        console.debug('Send Route KM512.R_CD TIMELOCKED - Real Route');
        //Send Route KM512.R_CD TIMELOCKED - Real Route
        sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 3, value: 2}]});
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send Route KM512.R_CD UNTIMELOCKED - Real Route');
          //Send Route KM512.R_CD UNTIMELOCKED - Real Route
      ...
      if(currentState === STATE['VIRTUAL_ROUTE_GRANTED']) {
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send Route KM512.R_CD AVAILABLE - Real Route');
          //Send Route KM512.R_CD AVAILABLE - Real Route
          sendChanges({section: 3, remote: 15, full: false, connected: true, changed: [{index: 2, value: 0}]});
