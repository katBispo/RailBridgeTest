...
const onDataReceived = (info) => {
  ...
  if(info.connected) {
    if(currentState === STATE['WAITING_CONNECTION']) {
      ...
      sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 50, value: 14}, {index: 52, value: 0}, {index: 53, value: 0}]});
      sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 2, value: 0}, {index: 2, value: 0}, {index: 3, value: 0}]});
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 0}, {index: 50, value: 14}, {index: 52, value: 0}, {index: 53, value: 0}]});
      ...
      sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 4, value: 64}]});
      sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 5, value: 32}]});
      ...
      // Route Granted
      // sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 52, value: 128}, {index: 50, value: 14}]});
    ...
    else if(info.section === 5 && info.remote === 12 && info.port === 2 && info.value === 2) {
      // Cancel Route
      ...
      //Send AMV6 and AMV7 Normal
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}, {index: 50, value: 14}, {index: 52, value: 0}]});
    }
    else if(info.section === 5 && info.remote === 12 && info.port === 26 && info.value === 1) {
      // Block

      console.debug('Block segment received !!!');

      // Send Block Segment
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 142}]});
    }
    else if(info.section === 5 && info.remote === 12 && info.port === 26 && info.value === 2) {
      // UnBlock

      console.debug('UnBlock segment received !!!');

      // Send UnBlockConf Segment
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 51, value: 1}]});
    }
    else if(info.section === 5 && info.remote === 12 && info.port === 26 && info.value === 4) {
      // UnBlock conf
    ...
    else if(info.section === 5 && info.remote === 12 && info.port === 1 && info.value === 32) {
      // Requested Route
      ...
      if(currentState === STATE['LOCKED_ROUTE']) {
        ...
        // sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 2, value: 4}]});
  
        seq_indications = 1;
        setTimeout(() => {
          console.debug('Send Route GRANTED - Virtual Route');
          //Send Route KM512.R_CE GRANTED - Virtual Route
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 2, value: 4}, {index: 52, value: 128}, {index: 50, value: 14}]});
        ...
        setTimeout(() => { 
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 12, value: 128}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 2, value: 0}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 12, value: 0}]});

          //Send Route AVAILABLE - Virtual Route
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}, {index: 52, value: 0}]});
      ...
      if(currentState === STATE['REAL_ROUTE_GRANTED']) {
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 52, value: 128}, {index: 50, value: 14}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 52, value: 0}, {index: 50, value: 15}]});
        }, INDICATION_INTERVAL * seq_indications);
      ...
      if(currentState === STATE['REAL_ROUTE_REGISTERED']) {
        ...
        sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 2, value: 8}]});
      ...
      if(currentState === STATE['REAL_ROUTE_TIMELOCKED']) {
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 3, value: 2}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 3, value: 0}]});
      ...
      if(currentState === STATE['VIRTUAL_ROUTE_AVAILABLE']) {
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send CDV 3E1T Occupied');
          //Send CDV 3E1T Occupied
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 46}]});

        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send CDV 3E1T Not Occupied');
          //Send CDV 3E1T Occupied
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV8 Not Normal');
          //Send AMV8 Not Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 10}]});

        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV8 Normal');
          //Send AMV8 Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV12 Not Normal');
          //Send AMV12 Not Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 6}]});

        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV12 Normal');
          //Send AMV12 Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV25 Not Normal');
          //Send AMV25 Not Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 12}]});

        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV25 Normal');
          //Send AMV25 Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});
        ...
        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV8, AMV12 and AMV25 Not Normal');
          //Send AMV8, AMV12 and AMV25 Not Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 0}]});

        }, INDICATION_INTERVAL * seq_indications);

        seq_indications++;
        setTimeout(() => {
          console.debug('Send AMV8, AMV12 and AMV25 Normal');
          //Send AMV8, AMV12 and AMV25 Normal
          sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});
