...
const onDataReceived = (info) => {
  ...
  if(info.connected) {
    if(currentState === STATE['WAITING_CONNECTION']) {
    ...
    else if(info.section === 5 && info.remote === 25 && info.port === 2 && info.value === 2) {
      // Cancel Route
    ...
    else if(info.section === 5 && info.remote === 25 && info.port === 1 && info.value === 32) {
      // Requested Route
      ...
      if(currentState === STATE['LOCKED_ROUTE']) {
        ...
        // sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 2, value: 4}]});
  
        seq_indications = 1;
        setTimeout(() => {
          console.debug('Send Route GRANTED - Virtual Route');
          //Send Route KM512.R_CE GRANTED - Virtual Route
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 2, value: 4}, {index: 52, value: 56}]});
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 2, value: 0}]});
        }, INDICATION_INTERVAL * seq_indications);
      ...
      if(currentState === STATE['REAL_ROUTE_REGISTERED']) {
        ...
        sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 2, value: 8}]});
      ...
      if(currentState === STATE['REAL_ROUTE_TIMELOCKED']) {
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 3, value: 2}]});
        }, INDICATION_INTERVAL * seq_indications);
