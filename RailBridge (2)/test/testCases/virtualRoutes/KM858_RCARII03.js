...
const onDataReceived = (info) => {
  ...
  if(info.connected) {
    if(currentState === STATE['WAITING_CONNECTION']) {
      ...
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 10, value: 64}]});
      ...
      //Send AMV6 and AMV7 Normal
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 11, value: 16}]});

      //Send AMV8, AMV12 and AMV25 Normal
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 50, value: 14}]});

      sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 51, value: 4}]});
      sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 51, value: 4}]});

      // sendChanges({section: 5, remote: 12, full: false, connected: true, changed: [{index: 15, value: 8}]});
    ...
    else if(info.section === 5 && info.remote === 25 && info.port === 1 && info.value === 1) {
      // Requested Route
      ...
      if(currentState === STATE['LOCKED_ROUTE']) {
        ...
        // sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 1}]});
  
        seq_indications = 1;
        setTimeout(() => {
          console.debug('Send Route GRANTED - Virtual Route');
          //Send Route KM512.R_CE GRANTED - Virtual Route
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 1}, {index: 52, value: 56}]});
      ...
      if(currentState === STATE['REAL_ROUTE_GRANTED']) {
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 52, value: 56}]});
        }, INDICATION_INTERVAL * seq_indications);
        ...
        setTimeout(() => {
          ...
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 52, value: 88}]});
        }, INDICATION_INTERVAL * seq_indications);
      ...
      if(currentState === STATE['REAL_ROUTE_REGISTERED']) {
        ...
        sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 2}]});
