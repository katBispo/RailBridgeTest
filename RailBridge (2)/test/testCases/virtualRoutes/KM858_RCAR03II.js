...
const onDataReceived = (info) => {
  ...
  if(info.connected) {
    if(currentState === STATE['WAITING_CONNECTION']) {
    ...
    else if(info.section === 5 && info.remote === 25 && info.port === 1 && info.value === 4) {
      // Requested Route
      ...
      if(currentState === STATE['LOCKED_ROUTE']) {
        ...
        // sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 16}]});
  
        seq_indications = 1;
        setTimeout(() => {
          console.debug('Send Route GRANTED - Virtual Route');
          //Send Route KM512.R_CE GRANTED - Virtual Route
          sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 16}, {index: 52, value: 152}]});
      ...
      if(currentState === STATE['REAL_ROUTE_REGISTERED']) {
        ...
        sendChanges({section: 5, remote: 25, full: false, connected: true, changed: [{index: 1, value: 32}]});
